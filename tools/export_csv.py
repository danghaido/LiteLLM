...
def export_evaluation_to_csv():
    ...
    all_spans_df = span_client.spans.get_spans_dataframe(
        query=SpanQuery(), project_identifier=PROJECT_NAME
    )

    # Get span annotations dataframe
    annotations_df = span_client.spans.get_span_annotations_dataframe(
        spans_dataframe=all_spans_df, project_identifier=PROJECT_NAME
    )

    # Process annotations dataframe
    annotations_df = annotations_df.reset_index().rename(  # Convert index to column
        columns={
    ...
    final_df["explanation"] = final_df["explanation"].replace("\n", "\\n", regex=True)
